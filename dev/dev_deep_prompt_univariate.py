"""
prefix tuning to train moment, nothing special
...
def train(model, train_loader, test_loader,
    ...
    for cur_epoch in range(max_epoch):
        ...
        loss_dict = {key: np.nanmean(
            [d[key] if key in d.keys() else np.nan for d in loss_dicts]) for key in loss_dicts[0].keys()}

        print(f"Epoch {cur_epoch}: Train loss: {average_loss:.3f}, Individual losses: {loss_dict}")

        wandb.log({
            'train_loss': average_loss} | {'train_'+key+'_loss': val for key, val in loss_dict.items()},
                  step=cur_epoch)
...
def prompt_tuning(train_loader, test_loader):
    ...
    wandb.init(
        project="ts-prompt",
        name="univariate_deep_prompt_tuning",
        )
